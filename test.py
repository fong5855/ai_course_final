try:
    ...
    '''model1 = modellib1.MaskRCNN(mode="training", config=config,
                              model_dir=MODEL_DIR)'''
    model1 = modellib1.MaskRCNN(mode="inference", config=config,
                              model_dir=MODEL_DIR)
    '''init_with = "last"
    ...
    model_path = "/mnt/logs/mask_rcnn_last_resnet32_origin.h5"
    ...
    original_image, image_meta, gt_class_id, gt_bbox, gt_mask =\
        modellib1.load_image_gt(dataset_val, config, 
                               image_id, use_mini_mask=False)

    t1 = log("original_image", original_image)
    t1 += log("image_meta", image_meta)
    t1 += log("gt_class_id", gt_class_id)
    t1 += log("gt_bbox", gt_bbox)
    t1 += log("gt_mask", gt_mask)

    visualize.display_instances(original_image, gt_bbox, gt_mask, gt_class_id, 
                                dataset_train.class_names, figsize=(8, 8), file_name="ori")


    results = model1.detect([original_image], verbose=1)

    #t1 += "feature map shape = " + str(model1.mrcnn_feature_maps[2].shape) + "\n"
    #t1 += str(model1.mrcnn_feature_maps[2]) + "\n"
    fmap1 = model1.detect_feature1([original_image], verbose=1)
    t1 += "feature map shape = " + str(fmap1.shape) + "\n"
    fig = plt.figure()
    ax1 = fig.add_subplot(211)
    ax2 = fig.add_subplot(212)
    
    ax1.imshow(fmap1[0,:,:,0])
    # t1 += str(fmap) + "\n"
    
    fmap2 = model1.detect_feature2([original_image], verbose=1)
    t1 += "feature map shape = " + str(fmap2.shape) + "\n"
    ax2.imshow(fmap2[0,:,:,0])
    
    fig.savefig("/mnt/feature_map.png")
    
    
    r = results[0]
    visualize.display_instances(original_image, r['rois'], r['masks'], r['class_ids'], 
                                dataset_val.class_names, r['scores'], ax=get_ax(), file_name="result")
    ...
    for image_id in image_ids:
        ...
        image, image_meta, gt_class_id, gt_bbox, gt_mask =\
            modellib1.load_image_gt(dataset_val, config,
                                   image_id, use_mini_mask=False)
        molded_images = np.expand_dims(modellib1.mold_image(image, config), 0)
    ...
    t1 += ("mAP: " + str(np.mean(APs)) + "\n")
    with open("/mnt/test.log", "w") as f:
        f.write(t1)
